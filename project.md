...
- [x] "LEARN" button added to Hero menu (10px gap, matching style)
- [x] Global state extended with `mode: 'LEARN'`
- [x] Screen A: 3D painting UI with guided sequence + palette limits
- [x] Screen B: Tutorial layout per reference image + step counter
- [x] Hybrid solver: npm package for Cross/F2L + custom 4LLL for OLL/PLL
- [x] Kociemba validation gate between Screen A → B
- [x] Interaction model: Click=Paint, Drag=Orbit (Input Phase)
- [x] Slice rotation enabled only in Tutorial/Practice phases
- [x] Top display repurposed as step tracker (visual parity preserved)
